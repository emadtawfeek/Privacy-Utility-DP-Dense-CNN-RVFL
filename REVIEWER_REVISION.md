...
> This is the earlier R1–R2 protocol record. For the isolated R3 package, use
> `RUN_OTHER_DEVICE.md` and `TECHNICAL_REPORT.md` first. Its references to
> `outputs_revision_full_plan` describe the **separate original project**; that
> plan and its running results are intentionally not present here. Running it
> here requires a fresh output directory and fresh manifest.
